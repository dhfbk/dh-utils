...
//            System.exit(1);
            ...
            Map<String, Map<String, FrequencyHashSet<String>>> aggregatedTotals = new HashMap<>();
            for (String value : years.values()) {
                if (value.trim().length() == 0) {
                    continue;
                }
                aggregatedTotals.putIfAbsent(value, new HashMap<>());
            }

            Map<String, Map<String, FrequencyHashSet<String>>> typeTotals = new HashMap<>();
            for (String value : schools.values()) {
                if (value.trim().length() == 0) {
                    continue;
                }
                typeTotals.putIfAbsent(value, new HashMap<>());
            }
