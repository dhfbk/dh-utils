...
public class OutputAnalyzerDec2020 {
...
//            taskToDo.add("27_LI");
            taskToDo.add("24_DISLOCAZIONI");
            taskToDo.add("25_FRASI_SCISSE");
...
//            System.exit(1);
            ...
            for (String taskName : files.keySet()) {
                ...
                for (String fileName : files.get(taskName).keySet()) {
                    ...
                    Map<Integer, List<String>> sentences = new TreeMap<>();
                    Map<Integer, Integer> tokenToSentence = new TreeMap<>();
...
//                    System.out.println(file.getAbsolutePath());
                    ...
                    for (int i = 0; i < nodeList.getLength(); i++) {
                        ...
                        String sentence_id = node.getAttributes().getNamedItem("sentence").getNodeValue();
                        int idInt = Integer.parseInt(id);
                        int sentenceInt = Integer.parseInt(sentence_id);
                        String value = node.getTextContent();
                        tokens.put(idInt, value);
                        if (!sentences.containsKey(sentenceInt)) {
                            sentences.put(sentenceInt, new ArrayList<>());
                        }
                        tokenToSentence.put(idInt, sentenceInt);
                        sentences.get(sentenceInt).add(value);
                        max = idInt;
                    }
...
//                    System.out.println(markablesTokens);
                    ...
                    switch (taskName) {
                        ...
                        case "25_FRASI_SCISSE":
                            Map<Integer, String> theseSentences = new HashMap<>();
                            Map<Integer, Set<String>> tipi = new HashMap<>();
                            for (Integer id : markablesInfo.keySet()) {
                                ...
                                for (Integer tokenID : tokenIDs) {
                                    Integer sentenceID = tokenToSentence.get(tokenID);
                                    if (!theseSentences.containsKey(sentenceID)) {
                                        theseSentences.put(sentenceID, String.join(" ", sentences.get(sentenceID)));
                                    }
                                    if (!tipi.containsKey(sentenceID)) {
                                        tipi.put(sentenceID, new HashSet<>());
                                    }
                                    if (tipo != null && tipo.length() > 0) {
                                        tipi.get(sentenceID).add(tipo);
                                    }
                                }
                            }
                            for (Integer sentenceID : theseSentences.keySet()) {
                                writer.append(file.getName()).append("\t")
                                        .append(taskName).append("\t")
                                        .append(tipi.size() > 0 ? String.join(",", tipi.get(sentenceID)) : "_").append("\t")
                                        .append(theseSentences.get(sentenceID)).append("\n");
                            }
